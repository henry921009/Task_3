...
Focus on properties that represent relationships between entities (e.g., P50 for 'author', P26 for 'spouse').
...
def fetch_subgraph_multihop(initial_qids: list, initial_pids: list, max_depth: int = 2):
    """
...
6. Use meaningful variable names that reflect the property they represent (e.g., ?dateOfBirth for P569).
...
if __name__ == "__main__":
    ...
    # 使用者問題
    question = "Who is the spouse of the director of the movie 'Titanic'?"
    ...
    # 根據實體名稱取得 QID
    print(get_wikidata_qid("The Old Man and the Sea"))  # 預期：Q26505
    print(get_wikidata_qid("Ernest Hemingway"))  # 預期：Q23434
    print(get_wikidata_qid("Barack Obama"))  # 預期：Q76
    print(get_wikidata_qid("nonexistent entity"))  # 預期：None
    qids_to_fetch = []
    ...
    if qids_to_fetch and relation_queries:
        triple_list = fetch_subgraph_multihop(
            qids_to_fetch, relation_queries, max_depth=2
        )
    else:
